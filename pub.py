...
def publish_messages_data(topic_num,delay_time,msg_num,pub_loop):
    message_data={}

    for i in range(1,pub_loop+1):
...
def publish_messages_status(topic_num,delay_time,pub_loop):
    message_status={}
    for i in range(1,pub_loop+1):
        for j in range(1, topic_num+1):  
...
def publish_messages_alarm(topic_num,delay_time,pub_loop):
    message_alarm={}
    for i in range(1,pub_loop+1):
        for j in range(1, topic_num+1):  
...
if __name__ == "__main__":
    try:
        publish_messages_data(100,1,10,10)
        publish_messages_status(100,1,5)
        publish_messages_alarm(100,1,5)
    finally:
        client.disconnect()
